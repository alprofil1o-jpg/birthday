...
export default function Dashboard({ birthday, onChangeBirthday }: DashboardProps) {
  ...
  const showCake = pullDistance > 30;

  return (
    <>
      {/* Custom pull-to-refresh indicator */}
      <div
        className="fixed top-0 left-0 right-0 z-50 flex items-center justify-center pointer-events-none overflow-hidden transition-all duration-200"
        style={{ height: pullDistance > 0 || isRefreshing ? Math.max(pullDistance, isRefreshing ? 60 : 0) : 0 }}
      >
        <div style={{
          fontSize: 32 + pullProgress * 16,
          opacity: pullProgress,
          transform: `rotate(${isRefreshing ? 'none' : pullProgress * 360 + 'deg'})`,
      ...
      <div
        className="min-h-screen p-4 pt-safe transition-colors duration-300 select-none"
        style={{
          background: isDark
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowSettings(false)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 w-80 shadow-2xl text-center" onClick={e => e.stopPropagation()}>
            ...
            <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 mb-4">
              <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold mb-1">Weboldalam</p>
              <p className="text-lg font-bold text-indigo-600 dark:text-indigo-400"><a href="https://balint12.hu" target="_blank" rel="noopener noreferrer">balint12.hu</a></p>
            </div>
